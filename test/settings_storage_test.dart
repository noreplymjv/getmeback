...
void main() {
  ...
  test('haptics and sfx default to enabled', () async {
    ...
    expect(StorageService.instance.reducedFxEnabled, isFalse);
  });
  ...
  test('setReducedFxEnabled persists', () async {
    await StorageService.instance.setReducedFxEnabled(true);
    expect(StorageService.instance.reducedFxEnabled, isTrue);
    await StorageService.instance.loadSettings();
    expect(StorageService.instance.reducedFxEnabled, isTrue);
  });

  test('loadJournalEntries parses mood + text', () async {
    await StorageService.instance.saveJournalEntry('felt lighter', mood: 'lighter');
    await StorageService.instance.saveJournalEntry('breathed out', mood: 'calm');
    ...
    expect(entries.first.mood, 'calm');
    expect(entries[1].text, 'felt lighter');
    expect(entries[1].mood, 'lighter');
    expect(
      entries.first.at.isBefore(DateTime.now().add(const Duration(minutes: 1))),
      isTrue,
    );
  });
